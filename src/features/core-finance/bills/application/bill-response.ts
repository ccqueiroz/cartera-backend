...
export interface BillWalletView {
  id: string;
  balance: number;
}

/** Envelope de liquidação de Bill, espelhando `CreateTransferResult` do transfer. */
export interface SettleBillResult {
  root: TransactionOutput;
  wallet: BillWalletView;
  warnings: BalanceWarning[];
}

/** Resultado de criação: só `root` quando a vencer; com wallet/warnings quando nasce paga. */
